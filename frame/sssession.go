...
func (s *SSSession) OnEstablish() {
	...
	if s.IsConnectType() {
		...
		s.AsyncSendProtoMsg(uint32(pb.S2SBaseMsgId_s2s_server_session_veriry_req_id), req)
		return
	}
}
...
func (s *SSSession) OnVerify() {
	...
	if s.IsConnectType() {
		s.timerRegister.AddRepeatTimer(SS_SEND_BEAT_HEART_TIMER_ID, SS_BEAT_SEND_HEART_DELAY, "SSSession-SendBeatHeart", func(v ...interface{}) {
			s.AsyncSendMsg(uint32(pb.S2SBaseMsgId_s2s_server_session_ping_id), nil)
...
func (s *SSSession) OnHandler(msgID uint32, datas []byte) {
	if msgID == uint32(pb.S2SBaseMsgId_s2s_server_session_veriry_req_id) && s.IsListenType() {
		...
		var VerifyResFunc = func(errorCode uint32) {
			...
			s.AsyncSendMsg(uint32(pb.S2SBaseMsgId_s2s_server_session_veriry_ack_id), nil)
		}
	...
	if msgID == uint32(pb.S2SBaseMsgId_s2s_server_session_ping_id) && s.IsListenType() {
		...
		s.AsyncSendMsg(uint32(pb.S2SBaseMsgId_s2s_server_session_pong_id), nil)
		return
	}
