...
// func (s *ServiceDiscoveryClient) Init(addr string, serverId uint64, token string, cb_func SDCbFunc) bool {
// 	s.addr = addr
// 	s.serverId = serverId
// 	s.token = token
// 	s.init_flag = false
...
// 			Token:    token,
// 		}
// 		s.ssclient_session.SendProtoMsg(uint32(framepb.S2SBaseMsgId_service_discovery_req_id), req)
// 		ELog.DebugAf("[ServiceDiscoveryClient] Send ServiceDiscoveryReq ServerID=%v", serverID)
// 	}, []interface{}{serverId, token}, true)
